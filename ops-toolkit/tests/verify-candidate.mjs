...
export function fingerprint(){
  ...
  const artifacts=Object.fromEntries(['../ops_toolkit_online.html','THIRD_PARTY_LICENSES.txt'].map(file=>[file,digest(readFileSync(path.join(root,file)))]));
...
if(!process.argv.includes('--fingerprint')){
  const commands=[['build.mjs'],['--test',...files('tests').filter(x=>x.endsWith('.test.mjs'))],['tests/manual-online-browser.mjs'],['tests/ui-boundaries.mjs'],['tests/environment-browser.mjs'],['tests/performance.mjs']];
