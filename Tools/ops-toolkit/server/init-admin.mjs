...
const db=openDatabase(resolve(process.env.OPS_TOOLKIT_DB || 'data/ops-toolkit.sqlite'));
try {
  if(db.prepare('SELECT COUNT(*) AS n FROM users').get().n)throw Error('管理员已初始化，无需重复创建');
  ...
  await new Auth(db).bootstrap(username,password);console.log('管理员已创建，首次登录需要修改密码。');
