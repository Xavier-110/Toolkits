...
function fresh(revision=0){return {schemaVersion:3,revision,dictionaries:{regions:[],environmentTypes:[],configNames:[]},bindings:[],configs:[],versions:[],legacy:[],recoveryDrafts:[],receipts:[],settings:{expiryWarningDays:30}};}
...
function validateLegacyPackage(pkg){
  if(!object(pkg)||pkg.schemaVersion!==3||pkg.exportKind!=='legacy'||pkg.redacted!==false||!Array.isArray(pkg.legacy)||!object(pkg.counts)||pkg.counts.legacy!==pkg.legacy.length)fail(400,'待映射包无效');
...
function backupValidate(pkg){
  if(!object(pkg)||pkg.schemaVersion!==3||pkg.redacted!==false||!['configs','versions'].includes(pkg.exportKind)||!object(pkg.dictionaries)||!Array.isArray(pkg.bindings)||!Array.isArray(pkg.configs)||!Array.isArray(pkg.versions))fail(400,'备份格式无效或脱敏包不可恢复');
...
export class Workspace {
  ...
  read(){const raw=JSON.parse(this.db.prepare('SELECT data FROM workspace WHERE id=1').get().data);if(raw.schemaVersion===3)return readSchema3(raw);if(![1,2].includes(raw.schemaVersion))fail(400,'不支持的工作空间版本');return transaction(this.db,()=>{const current=JSON.parse(this.db.prepare('SELECT data FROM workspace WHERE id=1').get().data);if(current.schemaVersion===3)return readSchema3(current);const state=migrated(current);this.db.prepare('UPDATE workspace SET data=? WHERE id=1').run(JSON.stringify(state));return state;});}
  ...
  execute(actor,command){
    ...
    const user=this.auth.requireUser(actor.id,'write'),time=new Date(this.clock()).toISOString();
    this.read();
    return transaction(this.db,()=>{
      let state=this.read(),data=command.data??{},result=null,changed=true;
      switch(command.type){
        case 'dictionary':{
          this.auth.requireUser(user.id,'admin');fields(data,['kind','id','code','label','enabled','remove','expectedRevision']);
        ...
        case 'binding':{
          this.auth.requireUser(user.id,'admin');fields(data,['regionId','environmentTypeId','configNameId','enabled','expectedRevision']);for(const key of ['regionId','environmentTypeId','configNameId'])id(data[key]);if(typeof data.enabled!=='boolean')fail(400,'启用状态无效');
        ...
        case 'bindings':{this.auth.requireUser(user.id,'admin');result=createBindings(state,data,user,time);break;}
        case 'deleteEnvironment':{this.auth.requireUser(user.id,'admin');result=deleteEnvironment(state,data,user,time);break;}
        case 'save':{
        ...
        case 'settings':{fields(data,['expiryWarningDays']);if(own(data,'expiryWarningDays')&&(!Number.isInteger(data.expiryWarningDays)||data.expiryWarningDays<0||data.expiryWarningDays>3650))fail(400,'设置无效');Object.assign(state.settings,data);result=state.settings;break;}
        case 'mapLegacy':{
          this.auth.requireUser(user.id,'admin');fields(data,['legacyId','regionId','environmentTypeId','configNameId']);validTriple(state,data);
        ...
        case 'import':{fields(data,['backup','skipConflicts','importSettings','copyMappings']);if(own(data,'skipConflicts')&&typeof data.skipConflicts!=='boolean'||own(data,'importSettings')&&typeof data.importSettings!=='boolean')fail(400,'冲突选项无效');const backup=data.backup;if(!object(backup)||![1,2,3].includes(backup.schemaVersion))fail(400,'不支持的备份版本');if(backup.schemaVersion!==3){if(data.copyMappings?.length)fail(400,'旧包需先进入待映射区');const old=validateBackup(JSON.stringify({...backup,schemaVersion:1}));const rows=legacyRows(old.workspace,{user,time});state.legacy.push(...rows);result={imported:0,legacy:rows.length};break;}
          ...
          backupValidate(backup);let imported=0,skipped=0;for(const kind of kinds)for(const d of backup.dictionaries[kind]){const local=state.dictionaries[kind].find(x=>x.id===d.id||x.code===d.code);if(local&&(local.id!==d.id||local.code!==d.code))fail(409,'字典冲突');if(!local){this.auth.requireUser(user.id,'admin');state.dictionaries[kind].push(pick(d,dictionaryKeys));}}
          for(const b of backup.bindings){if(state.bindings.some(x=>x.id===b.id&&triple(x)!==triple(b)))fail(409,'组合 ID 与现有组合冲突');if(!state.bindings.some(x=>triple(x)===triple(b))){this.auth.requireUser(user.id,'admin');state.bindings.push(pick(b,bindingKeys));}}
      ...
      if(changed)this.write(state);return {result:structuredClone(JSON.parse(JSON.stringify(result))),state};
    });
  }
  tick(){return 0;}
  export(actor,{kind='versions',regionId='',environmentTypeId='',redacted=false}={}){
    this.auth.requireUser(actor.id,'write');if(!['configs','versions'].includes(kind))fail(400,'导出类型无效');const state=this.read();const configs=state.configs.filter(c=>(!regionId||c.regionId===regionId)&&(!environmentTypeId||c.environmentTypeId===environmentTypeId));
    ...
    const pkg={schemaVersion:3,exportKind:kind,redacted,scope:{regionId,environmentTypeId},exportedAt:new Date(this.clock()).toISOString(),exportedBy:{id:actor.id,username:actor.username},dictionaries:Object.fromEntries(kinds.map(k=>[k,state.dictionaries[k].filter(d=>ids[k].has(d.id))])),bindings:state.bindings.filter(b=>configs.some(c=>triple(c)===triple(b))),configs:structuredClone(configs),versions:structuredClone(versions),recoveryDrafts:structuredClone(recoveryDrafts),settings:structuredClone(state.settings),counts:{configs:configs.length,versions:versions.length,recoveryDrafts:recoveryDrafts.length,archived:configs.filter(c=>c.archivedAt).length,unmapped:state.legacy.length}};
  ...
  backup(actor,id='',redacted=false){const pkg=this.export(actor,{kind:'versions',redacted});if(id){if(!pkg.configs.some(c=>c.id===id))fail(404,'配置不存在');pkg.configs=pkg.configs.filter(c=>c.id===id);pkg.versions=pkg.versions.filter(v=>v.configSetId===id);pkg.recoveryDrafts=pkg.recoveryDrafts.filter(r=>r.configSetId===id);const c=pkg.configs[0];pkg.bindings=pkg.bindings.filter(b=>triple(b)===triple(c));pkg.dictionaries.regions=pkg.dictionaries.regions.filter(x=>x.id===c.regionId);pkg.dictionaries.environmentTypes=pkg.dictionaries.environmentTypes.filter(x=>x.id===c.environmentTypeId);pkg.dictionaries.configNames=pkg.dictionaries.configNames.filter(x=>x.id===c.configNameId);pkg.counts={configs:1,versions:pkg.versions.length,recoveryDrafts:pkg.recoveryDrafts.length,archived:c.archivedAt?1:0,unmapped:pkg.counts.unmapped};delete pkg.digest;pkg.digest=digest(pkg);}return pkg;}
  legacyExport(actor){this.auth.requireUser(actor.id,'write');const legacy=this.read().legacy.map(cleanLegacy),pkg={schemaVersion:3,exportKind:'legacy',redacted:false,exportedAt:new Date(this.clock()).toISOString(),exportedBy:{id:actor.id,username:actor.username},legacy,counts:{legacy:legacy.length}};pkg.digest=digest(pkg);return pkg;}
}
