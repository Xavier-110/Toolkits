...
const result = await build({ entryPoints: [path.join(root, 'src/app.js')], bundle: true, minify: true, format: 'iife', platform: 'browser', target: ['chrome110', 'edge110', 'firefox115'], write: false, legalComments: 'inline', define: { __CERT_EXAMPLE__: JSON.stringify(rootCertificates[0]), __ONLINE__: String(online) } });
